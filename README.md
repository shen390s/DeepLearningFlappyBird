...
Sometime, we may need to trainning network in remote server machine, then we may need to use VNC server to avoid PyGame engine to talk to remote X server to speedup trainning work.

```shell
# setup DISPLAY and start VNC server
#
DISPLAY=:15
export DISPLAY
vncserver $DISPLAY
